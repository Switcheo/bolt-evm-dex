...
const COMPILED_LIBRARIES: { [key: string]: { content: string } } =
  COMPILED_LIB.sources;
const LIBRARIES_MAPPING: { [key: string]: string } = LIBS_MAPPING;

const findNestedImports = (
  filepath: string,
  ...
  if (
    !COMPILED_LIBRARIES[modifiedFilePath] ||
    processedFiles.has(modifiedFilePath)
  ) {
    return [];
  }
...
export const getCompilerInput = (code: string, tokenName: string) => {
  ...
  imports = imports.flatMap((importItem) => [
    importItem,
    ...findNestedImports(importItem, processedFiles),
  ]);
  ...
  imports.forEach((importPath) => {
    contractsInput[importPath] = {
      content: COMPILED_LIBRARIES[importPath].content,
    };
  });
