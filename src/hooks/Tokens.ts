...
} from "../store/modules/lists/hooks";
import { NEVER_RELOAD, useSingleCallResult } from "../store/modules/multicall/hooks";
import { useUserAddedTokens } from "../store/modules/user/hooks";
...
import { filterTokens } from "../utils/listFilter";
...
function useTokensFromMap(tokenMap: TokenAddressMap, includeUserAdded: boolean): { [address: string]: Token } {
  ...
  return useMemo(() => {
    ...
    const mapWithoutUrls = Object.keys(tokenMap[chainId as SupportedChainId]).reduce<{
      ...
      newMap[address] = tokenMap[chainId as SupportedChainId][address].token;
      return newMap;
    }, {});
...
export function useAllInactiveTokens(): { [address: string]: Token } {
  ...
  const activeTokensAddresses = Object.keys(useAllTokens()); // Error here 2
  const filteredInactive = activeTokensAddresses
    ? Object.keys(inactiveTokens).reduce<{ [address: string]: Token }>((newMap, address) => {
        if (!activeTokensAddresses.includes(address)) {
          newMap[address] = inactiveTokens[address as Address];
        }
        return newMap;
      }, {})
    : inactiveTokens;
